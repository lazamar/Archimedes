var Geo = (function(){
  ...
  function updateStatus(){
    var statusPromise = Promise.defer()
    var weatherUrl = 'http://api.openweathermap.org/data/2.5/weather?'+
    'units=metric&lat='+lat+'&lon='+lon+'&APPID='+AppKeys.weatherAPI;

    //Get get weather from lat and long
    var request = require('request');
    request.get(weatherUrl, function (error, responseObj, body){
      if (error) {
        console.error(error);
        statusPromise.reject(error);
      }
  ...
  function prepareResponseStatus(){
    return {
      ...
      'temp-max': status.main.temp_max,
      'temp-min': status.main.temp_min,
      'description': status.weather[0].description,
      'icon': codes[status.weather[0].id].icon,
      'lastCheck': lastCheck,
    };
  }
  var codes = {
      ...
      "731": {"label": "sand, t whirls", "icon": "cloudy-gusts"},
      ...
      "957": {"label": "high wind, r gale", "icon": "cloudy-gusts"},
